...
public class MobileRequestDispatcher extends Dispatcher {
	
	public MobileRequestDispatcher(HostDataBean hostData, String encoding,
		...
		try {
			...
			if(hostData != null) {
				...
				if(mappingY) {
					...
					try {
						byte[] body = Files.readAllBytes(Paths.get(hostData.getDocbase() + "mobile/" +request.getPath()));
